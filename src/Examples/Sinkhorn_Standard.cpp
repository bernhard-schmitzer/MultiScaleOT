...
int W2Grid() {
	...
	///////////////////////////////////////////////
	// problem setup
	TMultiScaleSetupGrid MultiScaleSetupX(&muX,depth);
	TMultiScaleSetupGrid MultiScaleSetupY(&muY,depth);
	...
	msg=epsScalingHandler.setupGeometricMultiLayerA(MultiScaleSetupX.nLayers,epsStart,epsTarget,epsSteps,muXdim[0],2.,layerCoarsest,true);
	if(msg!=0) { eprintf("error: %d\n",msg); return msg; }


	// errorGoal
	double errorGoal=1E-4;
	// other parameters	
...
int Interpolation() {
	...
	// (no need to setup radii)
	TMultiScaleSetup MultiScaleSetupX(posX.data,muXdat,posX.dimensions[0],posX.dimensions[1],depth,THierarchyBuilder::CM_Tree,true,true,false);
	TMultiScaleSetup MultiScaleSetupY(posY.data,muYdat,posY.dimensions[0],posY.dimensions[1],depth,THierarchyBuilder::CM_Tree,true,true,false);
	...
	TEpsScalingHandler epsScalingHandler;
	epsScalingHandler.setupGeometricSingleLayer(MultiScaleSetupX.nLayers,epsStart,epsTarget,epsSteps);
...
int WpSphere() {
	...
	TMultiScaleSetupSphere MultiScaleSetupX(posDat.data(),muXdat.data(),res,dim,depth);
	TMultiScaleSetupSphere MultiScaleSetupY(posDat.data(),muYdat.data(),res,dim,depth);
	...
	TEpsScalingHandler epsScalingHandler;
	msg=epsScalingHandler.setupGeometricMultiLayerA(MultiScaleSetupX.nLayers,epsStart,epsTarget,epsSteps,epsBoxScale,p,layerCoarsest,true);
	if(msg!=0) { eprintf("error: %d\n",msg); return msg; }
